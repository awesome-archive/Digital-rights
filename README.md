...
3. 加密还是匿名？  
这个问题背后似乎还是需求分析的问题。**加密可以让你的真正数据不被其他人看到，匿名是为了不让人知道这些数据是你的**。 一般认为，需要匿名的地方通常是需要在公共/半公共的场合发表言论/信息的地方（因为没有必要加密嘛）；而加密是为了在通讯中不被窃听，这通常是在个体私密的对话中进行（通常又反而需要确认双方的真实身份）。所以有时需求是矛盾的。当然也可能出现非常高安全的需求，那就两者都用上。   
